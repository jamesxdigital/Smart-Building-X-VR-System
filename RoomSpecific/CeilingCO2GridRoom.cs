...
public class CeilingCO2GridRoom : MonoBehaviour {
	...
	public TextMesh co2LeftTextMesh;
	public TextMesh co2RightTextMesh;
	...
	static public float[] calculateGrid(float co2) {
		if (co2 < 300f) {//very sparse grid
			...
			return gridSettings;
		}  else if (co2 > 500f && co2 < 600f) { //dence grid
	...
	public void ApplyGrid() { 
		...
		if (co2Toggle.isOn) {
			...
			GridLeft.SetFloat ("_GridLineThickness", calculateGrid (GetJsonDataRoom.co2left) [0]);
			GridLeft.SetFloat ("_GridLineSpacing", calculateGrid (GetJsonDataRoom.co2left) [1]);
			//right side grid obtaining CO2 data from right side CO2 sensor
			GridRight.SetFloat ("_GridLineThickness", calculateGrid (GetJsonDataRoom.co2right) [0]);
			GridRight.SetFloat ("_GridLineSpacing", calculateGrid (GetJsonDataRoom.co2right) [1]);
			//obtaining CO2 data and writting to 3D test in scene
			co2LeftTextMesh.text = GetJsonDataRoom.co2left.ToString ("0") + " PPM";
			co2RightTextMesh.text = GetJsonDataRoom.co2right.ToString("0") +" PPM";
			...
			//and remove all text
			co2LeftTextMesh.text = "";
			co2RightTextMesh.text = "";
		}
	}
